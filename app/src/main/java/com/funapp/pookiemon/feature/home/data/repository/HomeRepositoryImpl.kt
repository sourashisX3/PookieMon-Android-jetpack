package com.funapp.pookiemon.feature.home.data.repository

import android.util.Log
import com.funapp.pookiemon.core.config.network.ApiResult
import com.funapp.pookiemon.core.database.PokemonLocalDataSource
import com.funapp.pookiemon.feature.home.data.datasource.remote.PokemonRemoteDataSource
import com.funapp.pookiemon.feature.home.data.datasource.remote.toPokemonListItems
import com.funapp.pookiemon.feature.home.data.mapper.toDomain
...
class HomeRepositoryImpl @Inject constructor(
    ...
    override suspend fun getPokemonList(limit: Int, offset: Int, forceRefresh: Boolean): ApiResult<List<PokemonListItem>> {
        if (!forceRefresh) {
            ...
            if (cached != null) {
                Log.d("HomeRepository", "Cache hit for list offset=$offset")
                return ApiResult.success(cached)
            }
        }

        Log.d("HomeRepository", "Fetching network for list offset=$offset")
    ...
    override suspend fun getPokemonDetail(id: Int, forceRefresh: Boolean): ApiResult<Pokemon> {
        if (!forceRefresh) {
            ...
            if (cached != null) {
                Log.d("HomeRepository", "Cache hit for detail id=$id")
                return ApiResult.success(cached)
            }
        }

        Log.d("HomeRepository", "Fetching network for detail id=$id")
        return when (val result = remoteDataSource.getPokemonDetail(id)) {
            ...
            is ApiResult.Error -> {
                ...
                if (stale != null) {
                    Log.d("HomeRepository", "Stale cache fallback for detail id=$id")
    ...
    override suspend fun getPokemonSpecies(id: Int, forceRefresh: Boolean): ApiResult<PokemonSpecies> {
        if (!forceRefresh) {
            ...
            if (cached != null) {
                Log.d("HomeRepository", "Cache hit for species id=$id")
                return ApiResult.success(cached)
            }
        }

        Log.d("HomeRepository", "Fetching network for species id=$id")
        return when (val result = remoteDataSource.getPokemonSpecies(id)) {
            ...
            is ApiResult.Error -> {
                ...
                if (stale != null) {
                    Log.d("HomeRepository", "Stale cache fallback for species id=$id")
                    ApiResult.success(stale)
                } else result
            }
        }
